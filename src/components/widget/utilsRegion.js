...
export const parseSingleChart = (data, { calc, category_order }) => {
  const groupedData = groupBy(data, (d) => d.reduced_reporting_region);
  const regions = Object.keys(groupedData);
  const widgetData = regions.map((region) => {
    const arr = groupedData[region];
    const obj = {
      reduced_reporting_region: region,
    };
  ...
  return {
    config: {
      groupBy: 'reduced_reporting_region',
      categories,
    },
    data: widgetData,
  };
...
export const parseStackedChart = (data, { category_order }) => {
  ...
  const widgetData = answers.map((region) => {
    const arr = groupedData[region];
    const obj = {
      reduced_reporting_region: region,
    };
...
export const parseMultipleStackedChart = (data, { columns }) => {
  ...
  const wavesData = groupBy(parsedData, (d) => d.reduced_reporting_region);
  const widgetData = Object.keys(wavesData).map((waveKey) => {
    ...
    return {
      ...obj,
      reduced_reporting_region: waveKey,
    };
  });
...
export const getWidgetProps = (data, widgetSpec) => {
  ...
  // const waves = data.map((d) => d.reduced_reporting_region);
