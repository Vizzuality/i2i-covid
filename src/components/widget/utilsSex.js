...
export const parseSingleChart = (data, { calc, category_order }) => {
  const groupedData = groupBy(data, (d) => d.sex);
  const genders = Object.keys(groupedData);
  const widgetData = genders.map((gender) => {
    const arr = groupedData[gender];
    const obj = {
      sex: gender,
    };
...
export const parseStackedChart = (data, { category_order }) => {
  const groupedData = groupBy(data, (d) => d.sex);
  const genders = Object.keys(groupedData);
  const categories = category_order || uniq(map(data, 'sex').map((d) => String(d)));
...
export const parseMultipleStackedChart = (data, { columns }) => {
  ...
  const wavesData = groupBy(parsedData, (d) => d.sex);
